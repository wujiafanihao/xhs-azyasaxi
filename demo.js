...
`
    ...
    const download = async (urls, type_) => {
        ...
        const detail_info = extractDocument();
    ...
    const extractDescription = () => {
        ...
        // 遍历所有 meta 标签，查找 name 属性为 "description" 的标签
        ...
        // 如果没有找到 description 标签，返回 null 或其他默认值
        return null;
    };
    
    // 调用函数获取 description 内容
    ...
    //     if (appDiv) {
    //         console.log('找到 id 为 app 的 div 元素:', appDiv);
            
    //         const comment_user = appDiv.querySelector('#global div.main-container div.with-side-bar.main-content div.outer-link-container #noteContainer div.interaction-container div.note-scroller div.comments-el div.comments-container div.list-container div.parent-comment div.comment-item div.comment-inner-container div.right div.author-wrapper div.author a');
        
    //         console.log(comment_user);

    //     } else {
    //         console.log('未找到 id 为 app 的 div 元素');
    //     }
    // };
    // const extractComment = () => {
    //     const appDiv = document.getElementById('app');
    ...
    //         // 获取所有非 reply-container 的评论用户 a 标签
    //         const commentUsers = appDiv.querySelectorAll('div.parent-comment > div.comment-item:not(.reply-container) div.author a');
    //         const commentContent = appDiv.querySelectorAll('div.parent-comment > div.comment-item:not(.reply-container) div.content');
    //         const commentfram_info = appDiv.querySelectorAll('div.parent-comment > div.comment-item');
    
    //         console.log(commentUsers);
    //         console.log(commentContent);
    //         console.log(commentfram_info);

    
    //         // 遍历所有匹配的 a 标签并提取文本内容、href 和 data-user-id 属性
    //         commentUsers.forEach(a => {
    ...
    //             // 只输出用户名不为空的项
    //             if (username) {
    //                 console.log(`用户名: ${username}, 链接: ${href}, 用户ID: ${userId}`);
    //             }
    //         });
    //     } else {
    //         console.log('未找到 id 为 app 的 div 元素');
    ...
    const scrollAndExtractComments = (maxScrolls = 10) => {
        const appDiv = document.getElementById('app');
        ...
        if (appDiv) {
                ...
                commentItems.forEach(commentItem => {
                    ...
                    const extractData = (element) => {
                        element.childNodes.forEach(child => {
                            if (child.tagName === 'A' && child.className === 'name') {
                                username = child.innerText.trim();
                                href = child.getAttribute('href');
                            } else if (child.tagName === 'DIV' && child.className === 'date') {
                                date = child.innerText.trim();
                            } else if (child.tagName === 'DIV' && child.className === 'content') {
                    ...
                    // 开始递归遍历每个评论项
                    extractData(commentItem);
    
                    // 输出提取到的信息
                    if (username) {
                        console.log(`用户名: ${username}, 链接: ${href}, 日期: ${date}, 内容: ${content}`);
                    }
                });
            };
            ...
            // 设置定时器来定期滚动页面
            const scrollInterval = setInterval(scrollPage, 1000);
        } else {
            console.log('未找到 id 为 app 的 div 元素');
        }
    };
    
    // 调用函数并设置最大滚动次数，例如 20 次
    scrollAndExtractComments(20);
